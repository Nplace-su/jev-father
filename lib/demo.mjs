...
export function demoResult() {
  ...
  const final = { model: 'fixture / 非真实推理', answers: { tier: sampleAnswer(FINAL_QUESTION, '顶级') } };
  const repo = { fullName: '虚构示例 / jev-snack-bot', url: null, sha: 'demo', description: '一个让 Jev 选择零食、让现成控制器负责抓取的虚构小机器人。', totalFiles: 2,
  ...
  return { ...makeVerdict(dimensions, final), repo, demo: true, inferenceMs: null,
    payloads: [buildPayload(repo), buildFinalPayload(dimensions)], raw: [first, final] };
}
