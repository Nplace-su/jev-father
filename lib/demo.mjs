...
export function demoResult() {
  ...
  const picks = { fit: 'native', value: 'useful', substance: 'thin', honesty: 'grounded', robotics: 'demo' };
  const first = { model: 'fixture / 非真实推理', answers: Object.fromEntries(Object.entries(picks).map(([k, v]) => [k, sampleAnswer(questions[k], v)])) };
  const final = { model: 'fixture / 非真实推理', answers: { tier: sampleAnswer(FINAL_QUESTION, 'NPC') } };
