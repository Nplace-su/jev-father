...
export function readDimensions(response) {
  ...
  return AXES.map(axis => {
    ...
    return { id: axis.id, name: axis.name, choice: a.choice, confidence: a.confidence,
      probabilities: a.probabilities, known: !!option, uncertain: !confident, skip,
...
export function makeVerdict(dimensions, response) {
  ...
  return { ...tier, dimensions, confidence: answer.confidence,
    probabilities: Object.fromEntries(TIERS.map(t => [t.label, answer.probabilities[t.label]])), uncertain: answer.confidence < CONFIDENCE_FLOOR || dimensions.filter(d => d.known).length < 3,
